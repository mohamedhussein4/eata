...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-3xl p-6 lg:p-8 shadow-sm border border-gray-100">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl lg:text-3xl font-bold text-gray-900">عرض رأي #{{ $testimonial->id }}</h1>
                <p class="text-gray-600 mt-2">عرض تفاصيل رأي المستخدم</p>
                {{-- Breadcrumbs --}}
                <nav class="flex items-center space-x-2 {{ app()->getLocale() === 'ar' ? 'space-x-reverse' : '' }} mt-4">
                    ...
                    <i class="fas fa-chevron-{{ app()->getLocale() === 'ar' ? 'left' : 'right' }} text-gray-400 text-xs"></i>
                    <span class="text-gray-700 font-medium">عرض رأي</span>
                </nav>
            </div>
            {{-- Action Buttons --}}
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.testimonials.edit', $testimonial->id) }}" class="inline-flex items-center justify-center px-4 py-2.5 text-sm font-medium text-indigo-700 bg-indigo-100 hover:bg-indigo-200 rounded-2xl transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 lg:p-8">
            ...
            <div class="flex items-start gap-4 mb-6 pb-6 border-b border-gray-200">
                ...
                <div class="flex-1">
                    <h2 class="text-xl font-bold text-gray-900 mb-1">معلومات الرأي</h2>
                    <p class="text-gray-600">تفاصيل رأي المستخدم</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-gray-50 rounded-2xl p-6">
                    ...
                    <dl class="space-y-4">
                        <div>
                            <dt class="text-sm font-medium text-gray-500">رقم الرأي</dt>
                            <dd class="mt-1 text-sm text-gray-900">#{{ $testimonial->id }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">الاسم</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $testimonial->name }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">المسمى الوظيفي</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $testimonial->title ?? 'غير محدد' }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">تاريخ الإنشاء</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $testimonial->created_at->format('Y-m-d H:i:s') }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                <div class="bg-gray-50 rounded-2xl p-6">
                    ...
                    <dl class="space-y-4">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">{{ $testimonial->updated_at->format('Y-m-d H:i:s') }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
