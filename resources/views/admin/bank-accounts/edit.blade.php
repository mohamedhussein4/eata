...
@section('page-title', 'تعديل الحساب البنكي')
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-3xl p-6 lg:p-8 shadow-sm border border-gray-100">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl lg:text-3xl font-bold text-gray-900">تعديل الحساب البنكي</h1>
                <p class="text-gray-600 mt-2">تعديل بيانات الحساب البنكي: {{ $bankAccount->account_name ?? $bankAccount->bank_name }}</p>
                ...
                <a href="{{ route('admin.bank-accounts.index') }}" class="text-gray-500 hover:text-gray-700 transition-colors duration-200">الحسابات البنكية</a>
                    <i class="fas fa-chevron-{{ app()->getLocale() === 'ar' ? 'left' : 'right' }} text-gray-400 text-xs"></i>
                    <span class="text-gray-700 font-medium">تعديل الحساب</span>
                </nav>
            </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('admin.bank-accounts.update', $bankAccount->id) }}" method="POST" class="p-6 lg:p-8 space-y-6">
            @csrf
            @method('PUT')

            {{-- Form Header --}}
            <div class="border-b border-gray-200 pb-6">
                <h2 class="text-xl font-bold text-gray-900 mb-2">تعديل معلومات الحساب البنكي</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <select id="currency" name="currency" required class="block w-full px-4 py-3 border-2 border-gray-200 rounded-2xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-300">
                        ...
                        <option value="SYP" {{ ($bankAccount->currency ?? '') == 'SYP' ? 'selected' : '' }}>ليرة سورية (SYP)</option>
                        <option value="USD" {{ ($bankAccount->currency ?? '') == 'USD' ? 'selected' : '' }}>دولار أمريكي (USD)</option>
                        <option value="EUR" {{ ($bankAccount->currency ?? '') == 'EUR' ? 'selected' : '' }}>يورو (EUR)</option>
                        <option value="EGP" {{ ($bankAccount->currency ?? '') == 'EGP' ? 'selected' : '' }}>جنيه مصري (EGP)</option>
                        <option value="SAR" {{ ($bankAccount->currency ?? '') == 'SAR' ? 'selected' : '' }}>ريال سعودي (SAR)</option>
                        <option value="AED" {{ ($bankAccount->currency ?? '') == 'AED' ? 'selected' : '' }}>درهم إماراتي (AED)</option>
                        <option value="JOD" {{ ($bankAccount->currency ?? '') == 'JOD' ? 'selected' : '' }}>دينار أردني (JOD)</option>
                        <option value="LBP" {{ ($bankAccount->currency ?? '') == 'LBP' ? 'selected' : '' }}>ليرة لبنانية (LBP)</option>
                    </select>
                    @error('currency')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="space-y-2">
                ...
                <textarea id="description" name="description" rows="4"
                          ...
                          placeholder="أدخل أي معلومات إضافية عن الحساب (مثل: نوع الحساب، الغرض من استخدامه، إلخ...)">{{ old('description', $bankAccount->description ?? '') }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center">
                <input type="checkbox" id="is_active" name="is_active" value="1"
                       {{ ($bankAccount->is_active ?? true) ? 'checked' : '' }}
    ...
    <div class="bg-red-50 border border-red-200 rounded-3xl p-6">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                ...
                <p class="text-red-700 text-sm mb-4">حذف الحساب البنكي عملية غير قابلة للتراجع. سيتم حذف جميع البيانات المرتبطة به نهائياً.</p>
                <form action="{{ route('admin.bank-accounts.destroy', $bankAccount->id) }}" method="POST" class="inline-block">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                            onclick="return confirm('هل أنت متأكد من حذف هذا الحساب؟ هذا الإجراء لا يمكن التراجع عنه!')"
