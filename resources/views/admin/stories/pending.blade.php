...
@section('page-title', 'القصص المعلقة')
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-3xl p-6 lg:p-8 shadow-sm border border-gray-100">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl lg:text-3xl font-bold text-gray-900">القصص المعلقة</h1>
                <p class="text-gray-600 mt-2">عرض وإدارة القصص التي تحتاج للمراجعة والموافقة</p>
                {{-- Breadcrumbs --}}
                <nav class="flex items-center space-x-2 {{ app()->getLocale() === 'ar' ? 'space-x-reverse' : '' }} mt-4">
                    ...
                    <i class="fas fa-chevron-{{ app()->getLocale() === 'ar' ? 'left' : 'right' }} text-gray-400 text-xs"></i>
                    <span class="text-gray-700 font-medium">القصص المعلقة</span>
                </nav>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300">
            ...
            <div class="relative">
                @if($story->image)
                    <img src="{{ asset('/' . $story->image) }}" alt="{{ $story->title }}" class="w-full h-48 object-cover">
                @else
            ...
            <div class="p-6">
                ...
                <div class="flex gap-2">
                    ...
                    <form action="{{ route('admin.stories.reject', $story->id) }}" method="POST" class="flex-1">
                        @csrf
                        <button type="submit" class="w-full inline-flex items-center justify-center px-3 py-1.5 text-xs font-medium text-red-700 bg-red-100 hover:bg-red-200 rounded-lg transition-colors duration-200" onclick="return confirm('هل أنت متأكد من رفض هذه القصة؟')">
        ...
        <div class="col-span-full">
            <div class="text-center bg-white rounded-3xl p-12 shadow-sm border border-gray-100">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-3">لا توجد قصص معلقة</h3>
                <p class="text-gray-600 mb-8 max-w-md mx-auto">جميع القصص تمت مراجعتها والموافقة عليها.</p>
                <a href="{{ route('admin.stories.index') }}" class="inline-flex items-center justify-center px-6 py-3 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-2xl transition-all duration-300 shadow-sm hover:shadow-md">
