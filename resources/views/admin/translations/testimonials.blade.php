...
@section('page-title', 'ترجمة آراء العملاء')
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-3xl p-6 lg:p-8 shadow-sm border border-gray-100">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl lg:text-3xl font-bold text-gray-900">ترجمة آراء العملاء</h1>
                <p class="text-gray-600 mt-2">إدارة ترجمة جميع آراء العملاء في النظام</p>
                ...
                <nav class="flex items-center space-x-2 {{ app()->getLocale() === 'ar' ? 'space-x-reverse' : '' }} mt-4">
                    ...
                    <i class="fas fa-chevron-{{ app()->getLocale() === 'ar' ? 'left' : 'right' }} text-gray-400 text-xs"></i>
                    <span class="text-gray-700 font-medium">الآراء</span>
                </nav>
            </div>

            {{-- Action Buttons --}}
            <div class="flex items-center gap-3">
                <button onclick="syncTestimonialTranslations()" class="inline-flex items-center justify-center px-4 py-2.5 text-sm font-medium text-blue-700 bg-blue-100 hover:bg-blue-200 rounded-2xl transition-all duration-300">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-500">إجمالي الآراء</p>
                    <p class="text-2xl font-bold text-gray-900">{{ $testimonials->total() ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600">{{ $testimonials->where('is_translated', true)->count() ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-orange-600">{{ $testimonials->where('is_translated', false)->count() ?? 0 }}</p>
                </div>
                <div class="w-12 h-12 bg-gradient-to-r from-orange-500 to-orange-600 rounded-full flex items-center justify-center">
    ...
    {{-- Filters --}}
    <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        <form method="GET" action="{{ route('admin.translations.testimonials') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            {{-- Search --}}
            <div>
            ...
            <div class="flex items-end gap-3">
                ...
                <a href="{{ route('admin.translations.testimonials') }}" class="inline-flex items-center justify-center px-4 py-3 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-2xl transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="{{ app()->getLocale() === 'ar' ? 'mr-4' : 'ml-4' }}">
                                        <div class="text-sm font-medium text-gray-900">#{{ $testimonial->id }}</div>
                                        <div class="text-sm text-gray-500">{{ Str::limit($testimonial->name, 20) }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">{{ $testimonial->name }}</div>
                                <div class="text-sm text-gray-500">{{ $testimonial->name_en ?? '' }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">{{ Str::limit($testimonial->content, 80) }}</div>
                                <div class="text-sm text-gray-500">{{ Str::limit($testimonial->content_en ?? '', 80) }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    @for($i = 1; $i <= 5; $i++)
                                        ...
                                        @endif
                                    @endfor
                                    <span class="text-sm text-gray-500 {{ app()->getLocale() === 'ar' ? 'mr-2' : 'ml-2' }}">({{ $testimonial->rating }})</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($testimonial->is_translated ?? false)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.translations.testimonials.edit', $testimonial->id) }}" 
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 mb-2">لا توجد آراء</h3>
                                    <p class="text-gray-500">لم يتم العثور على أي آراء للترجمة.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
<script>
function syncTestimonialTranslations() {
    if (confirm('هل أنت متأكد من مزامنة ترجمات الآراء؟')) {
        // يمكن إضافة منطق المزامنة هنا
        alert('تم مزامنة ترجمات الآراء بنجاح');
        location.reload();
    }
}
</script>
