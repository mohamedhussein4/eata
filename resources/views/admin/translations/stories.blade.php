...
@section('page-title', 'ترجمة القصص')
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-3xl p-6 lg:p-8 shadow-sm border border-gray-100">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl lg:text-3xl font-bold text-gray-900">ترجمة القصص</h1>
                <p class="text-gray-600 mt-2">إدارة ترجمة جميع قصص المستفيدين في النظام</p>
                ...
                <nav class="flex items-center space-x-2 {{ app()->getLocale() === 'ar' ? 'space-x-reverse' : '' }} mt-4">
                    ...
                    <i class="fas fa-chevron-{{ app()->getLocale() === 'ar' ? 'left' : 'right' }} text-gray-400 text-xs"></i>
                    <span class="text-gray-700 font-medium">القصص</span>
                </nav>
            </div>

            {{-- Action Buttons --}}
            <div class="flex items-center gap-3">
                <button onclick="syncStoryTranslations()" class="inline-flex items-center justify-center px-4 py-2.5 text-sm font-medium text-blue-700 bg-blue-100 hover:bg-blue-200 rounded-2xl transition-all duration-300">
    ...
    {{-- Filters --}}
    <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        <form method="GET" action="{{ route('admin.translations.stories') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            {{-- Search --}}
            <div>
            ...
            <div class="flex items-end gap-3">
                ...
                <a href="{{ route('admin.translations.stories') }}" class="inline-flex items-center justify-center px-4 py-3 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-2xl transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="{{ app()->getLocale() === 'ar' ? 'mr-4' : 'ml-4' }}">
                                        <div class="text-sm font-medium text-gray-900">#{{ $story->id }}</div>
                                        <div class="text-sm text-gray-500">{{ Str::limit($story->title, 20) }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 mb-2">لا توجد قصص</h3>
                                    <p class="text-gray-500">لم يتم العثور على أي قصص للترجمة.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
<script>
function syncStoryTranslations() {
    if (confirm('هل أنت متأكد من مزامنة ترجمات القصص؟')) {
        // يمكن إضافة منطق المزامنة هنا
        alert('تم مزامنة ترجمات القصص بنجاح');
        location.reload();
    }
}
</script>
